(() => {
    ...
    const fetchGameData = (gameSlug) => {
        ...
        return new Promise(resolve => {
            fetch(`bad-ice-cream-3.json?https://api.poki.com/game/${gameSlug}?site=${siteId}`)
                .then(response => response.json())
                .then(data => {
                    resolve({
                        thumb: `thumbs.png?https://img.poki.com/cdn-cgi/image/quality=78,width=${size},height=${size},fit=cover,g=0.5x0.5,f=auto/${data.image.path}`,
                        title: data.title
                    });
                });
        });
